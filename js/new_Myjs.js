...
function make_table(n) {
	...
	//打印表格第一行（有星期标志）
	var table_s = "<table><thead><tr><td colspan='2'><a href='javascript:make_table(-1)' class='previous-month'>上一月</a></td><td colspan='3'><b class='current-month'>" + ynow + "年" + (mnow + 1) + "月</b></td><td colspan='2'><a href='javascript:make_table(1)' class='next-month'>下一月</a></td></tr><tr class='title'><td>日</td><td>一</td><td>二</td><td>三</td><td>四</td><td>五</td><td>六</td></tr></thead><tbody>";
	...
	for (i = 0; i < tr_str; i++) {
		...
		for (k = 0; k < 7; k++) {
			...
			current_cell_id = "cell_" + ynow + "-" + tmp_m + "-" + date_str;
...
//    var date_return = document.getElementById("TOURDATE_return");
    ...
    for(var i = 0; i < l; i++){
        ...
        if(y == tmp_1[0] && m == tmp_1[1] && d == tmp_1[2]){
			var x = tmp[1].split('+');
            var href_r = href_order;
            var href_2 = href_shopcar;

            // 把SKU_ID加到订购链接字符串的最后面
            //var new_href = href_r[0] + '/' + href_r[1] + '/' + href_r[2] + '/' + href_r[3] + '/' + href_r[4] + '/' + href_r[5] + '/' + x[4];
            //alert(order_button);
            if (get_stock(y,m,d) != '0') {
                var new_href = href_r + '?skuid=' + x[4];
                var new_href2 = href_2 + '?skuid='+ x[4];

                order_button.href = new_href;
                order_button2.href = new_href2;
                $('#order_button').css({'backgroundColor':'#F13F4C','color':'#F6F6F6'});
                $('#order_button2').css({'backgroundColor':'#F13F4C','color':'#F6F6F6'});
            } else{
                //alert('aaa');
                ...
                $('#order_button').css({'backgroundColor':'#F6F6F6','color':'#F6F6F6','cursor':'pointer'});
                $('#order_button2').css({'backgroundColor':'#F6F6F6','color':'#F6F6F6','cursor':'pointer'});
...
//点击详情页日历有有效时价的单元格，背景变色。
function dateClickColor(){
	$("#cal_div table tbody td").click(function() {
		var a = $(this).find(".val").html();
		if (a !== '') {
			$("#cal_div table tbody td").css("background", "");
			$(this).css("background", "#eb8860");
		}
	});
	var selectdate = document.getElementById("TOURDATE").innerHTML;
	$("#cell_"+selectdate).css("background", "#ffcc66");
}